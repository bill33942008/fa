    ...
    *,
    ...
    if post_format == "short_video_script":
        ...
        format_requirements = textwrap.dedent(
            ...
            - 文末要有收藏、评论或提问引导。
            - 配图位置：用「【配图1】」标注位置即可，不要额外写配图说明或配图建议。
            """
        ).strip()
        ...
        format_requirements = textwrap.dedent(
            ...
            - 时间校准：文中所有时间表述必须写明具体北京时间日期+时间（如"7月3日早上7:00"），不得只用"今晚""明日""凌晨"等模糊表述。
            - 如果选题中提到了比赛日期，请务必用该日期对应到北京时间进行描述。
    ...
    user_prompt = textwrap.dedent(
        ...
        - 如果选题标题明确是即将开始的比赛（含日期、对阵双方），必须围绕该比赛做深度分析，
          不得写成通用风险提示或信息核实清单。
        - 当前北京时间：{now_local().strftime('%Y年%m月%d日 %H:%M')}（星期{now_local().strftime('%A')}）。
          请基于当前时间校准所有时效表述。如果文中涉及比赛时间，必须写明具体北京时间日期+时间（如"7月3日早上7:00"），
          不得只用"今晚""明日""凌晨"等模糊表述。
